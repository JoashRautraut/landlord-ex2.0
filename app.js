...
function resetModalToStep1() {
  ...
  const summaryContainer = document.getElementById('points-summary');
...
function updateButtons() {
  ...
  const addToMapBtn = document.getElementById('add-survey-to-map-btn');
  ...
  // Next/Add button
  if (nextBtn && addToMapBtn) {
    if (currentStep < totalSteps) {
      nextBtn.style.display = 'flex';
      addToMapBtn.style.display = 'none';
    } else {
      nextBtn.style.display = 'none';
      addToMapBtn.style.display = 'flex';
    }
  }
...
function validateCurrentStep() {
  ...
  if (addToMapBtn) addToMapBtn.disabled = !isValid;
}
...
function generatePointFields() {
  ...
  const container = document.getElementById('dynamic-points-inputs');
  const pointsInputContainer = document.getElementById('points-input-container');
  
  if (!container || !pointsInputContainer) return;
  ...
  for (let i = 1; i <= pointsCount; i++) {
    ...
    const eastingInput = pointRow.querySelector(`#point-${i}-easting`);
    const northingInput = pointRow.querySelector(`#point-${i}-northing`);
    
    [eastingInput, northingInput].forEach(input => {
      input.addEventListener('input', collectAndValidatePoints);
    });
  }
  
  pointsInputContainer.style.display = 'block';
  validateCurrentStep();
}
...
function collectAndValidatePoints() {
  ...
  for (let i = 1; i <= pointsCount; i++) {
    const eastingEl = document.getElementById(`point-${i}-easting`);
    const northingEl = document.getElementById(`point-${i}-northing`);
    
    if (!eastingEl || !northingEl) continue;
    
    const easting = eastingEl.value.trim();
    const northing = northingEl.value.trim();
    
    if (easting && northing && !isNaN(parseFloat(easting)) && !isNaN(parseFloat(northing))) {
  ...
  if (collectedPoints.length === pointsCount && allValid) {
...
function calculateSummaryFromPoints() {
  ...
  const allPoints = [startPoint, ...collectedPoints];
  const latLngPoints = allPoints.map(point => transformSurveyToLatLng(point.easting, point.northing));
  const area = calculatePolygonArea(latLngPoints);
  ...
  if (totalPointsEl) totalPointsEl.textContent = collectedPoints.length + 1;
  if (totalDistanceEl) totalDistanceEl.textContent = `${totalDistance.toFixed(2)} m`;
  if (totalAreaEl) totalAreaEl.textContent = `${area.toFixed(2)} m²`;
}

// Override the existing modal functions
function showBearingsModal() {
  showBearingsModalMultiStep();
}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (generateBtn) {
    generateBtn.onclick = generatePointFields;
  }
  
  // Points count input validation
  const pointsCountInput = document.getElementById('points-count');
  if (pointsCountInput) {
    pointsCountInput.addEventListener('input', function() {
      const count = parseInt(this.value);
      const generateBtn = document.getElementById('generate-points-btn');
      if (generateBtn) {
        generateBtn.disabled = !count || count < 3 || count > 20;
      }
    });
  }
  ...
  const northingInput = document.getElementById('starting-northing');
  if (eastingInput && northingInput) {
    eastingInput.addEventListener('input', validateCurrentStep);
    northingInput.addEventListener('input', validateCurrentStep);
  }
  
  // Override add to map functionality to use collected points
  const addToMapBtn = document.getElementById('add-survey-to-map-btn');
  if (addToMapBtn) {
    addToMapBtn.onclick = function() {
      // Convert collected points to the format expected by existing function
      surveyPoints = collectedPoints.map((point, index) => {
        let prevPoint;
        if (index === 0) {
          prevPoint = {
            easting: parseFloat(document.getElementById('starting-easting').value),
          ...
          prevPoint = collectedPoints[index - 1];
        }
      ...
      // Call the existing add survey function
      addSurveyToMap();
    };
  }
});